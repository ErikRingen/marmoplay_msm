fig_segment_example <- function(post, d, standat){
  ...
  p <- ggplot(filter(long_df2, session_id %in% c("Washington: session 10")  & !is.na(type))) +
  ...
  ggsave("fig/fig3_b.pdf", height = 5, width = 11, dpi = 900)
