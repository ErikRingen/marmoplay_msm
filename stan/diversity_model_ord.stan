...
model{
  ...
  for (n in 1:N) {
    if (signal_before[n] == 0) n_cat[n] ~ ordered_logistic( session_z[session_id[n], 1]*sigma_session[1], c_nosignal);
    
    else if (signal_before[n] == 1) n_cat[n] ~ ordered_logistic( session_z[session_id[n], 1]*sigma_session[1] + (session_z[session_id[n], 2]*sigma_session[2] )*signal_before[n], c_signal);
;
  }
}
